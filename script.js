...
function buildPdfHtml(data, logoDataUrl) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>'
    + '@page{size:A5 portrait;margin:0;}html,body{width:148mm;height:210mm;margin:0;padding:0;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;}body{font-family:Arial,Helvetica,sans-serif;color:#1a1a1a;}'
    + '.page{width:148mm;height:210mm;overflow:hidden;padding:5mm 5mm;box-sizing:border-box;background:#fff;}'
    + '.header{display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #1a5c2e;padding-bottom:7px;margin-bottom:7px;}'
    + '.logo{width:62px;height:62px;object-fit:contain;}'
    + '.title-block{text-align:center;flex:1;padding:0 10px;}'
    + '.title-block h1{font-size:18px;font-weight:900;color:#1a5c2e;letter-spacing:3px;margin:0;text-transform:uppercase;}'
    + '.title-leaf{color:#2d7a45;font-size:12px;margin:2px 0;}'
    ...
    + '.member-box{background:#e8f5ec;border:1px solid #b0cfb8;border-radius:6px;padding:7px 9px;margin-bottom:7px;display:flex;justify-content:space-between;align-items:center;}'
    ...
    + 'th{background:#2a6b3c;color:#fff;font-size:9px;font-weight:700;padding:4px 4px;text-align:center;border-right:1px solid rgba(255,255,255,0.3);}'
    + 'th:last-child{border-right:none;}'
    + 'td{border:1px solid #c0d0b8;padding:3px 4px;font-size:9.5px;height:8mm;}'
    ...
    + '.pay-section{background:#e8f5ec;border:1px solid #b0cfb8;border-radius:6px;padding:7px 9px;margin-bottom:7px;display:flex;align-items:center;gap:20px;}'
    ...
    + '.partner-box{border-bottom:1px solid #555;margin-bottom:10px;padding:5px 0;display:flex;align-items:center;gap:8px;font-size:12px;font-weight:700;}'
...
function openPrintWindow(data, htmlContent, loader) {
  ...
  if (!win) {
    showToast('⚠️ Please allow pop-ups to open the A5 invoice preview');
    return;
  }
  win.document.open();
  ...
  showToast('📄 A5 invoice ready — choose Save as PDF or Print');
}

/* Share workflow: generate a clean A5 print preview. No HTML file is shared. */
...
function downloadPdf() {
  ...
  let billNo = document.getElementById('bill_no').textContent.trim();
  if (!billNo || billNo === '— generated on submit —') {
